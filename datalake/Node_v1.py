...
def parse_text(text):
    ...
    if matches:
        ...
        if operator == 'NOT':
            ...
            if node.left.operator == 'NOT':  # Ignoriere doppelte NOT-Knoten
                node.left = node.left.left
        else:
            node.left = parse_text(before)
            node.right = parse_text(after)
            if not node.right:  # Ergänze den fehlenden Partner
                node.right = Node(criteria="")
            if not node.left:  # Ergänze den fehlenden Partner
                node.left = Node(criteria="")
        return node
    else:
